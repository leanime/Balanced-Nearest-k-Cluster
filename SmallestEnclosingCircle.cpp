...
Circle makeDiameter(const Point &a, const Point &b) {
    Point c{(a.coords[0] + b.coords[0]) / 2, (a.coords[1] + b.coords[1]) / 2};
...
Circle makeCircumcircle(const Point &a, const Point &b, const Point &c) {
    ...
    double ox = (std::min(std::min(a.coords[0], b.coords[0]), c.coords[0]) + std::max(std::min(a.coords[0], b.coords[0]), c.coords[0])) / 2;
    double oy = (std::min(std::min(a.coords[1], b.coords[1]), c.coords[1]) + std::max(std::min(a.coords[1], b.coords[1]), c.coords[1])) / 2;
    double ax = a.coords[0] - ox, ay = a.coords[1] - oy;
    double bx = b.coords[0] - ox, by = b.coords[1] - oy;
    double cx = c.coords[0] - ox, cy = c.coords[1] - oy;
